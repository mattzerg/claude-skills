...
description: Hygiene scan of Matt's 2 memory scopes (core = env+personal in ~/dotfiles/claude-memory; work = Zerg in MattZerg/claude-memory — post-2026-06-30 collapse) plus the CLAUDE.md files — flags index bloat, orphaned memory files (exist but not in MEMORY.md), dangling index entries (point to missing files), oversized single memories, stale/long-untouched files, and broken [[wikilinks]]. Advisory only — proposes prunes/merges/fixes for confirmation, never auto-deletes. Use when Matt says "check my memory", "is my memory getting bloated", "audit/prune memory", "clean up MEMORY.md", or on a periodic cadence. Lane-aware (Obsidian wikilinks resolve vault-wide; the Zerg index legitimately projects MattZerg/_agent_memory).
---

# memory-maintenance

Keeps the 2-scope markdown memory system (core + work) lean and self-consistent. Complements (does not replace)
