...
# gmail-skill needs system Python (google-auth deps); this miner runs under its own
# .venv (for mlx_whisper/gallery-dl), so sys.executable is the venv python that LACKS
# those deps — using it made every `harvest` fail with an empty-stdout JSON error, so
# the cursor was never created. Resolve a system python3 for the gmail-skill subprocess.
